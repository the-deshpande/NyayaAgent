...
def _entry_route(state: NyayaState) -> str:
    query = (state.get("query") or "").strip().lower()
    raw_query = query.split("]\n")[-1].strip() if "]\n" in query else query
    
    # Quick heuristics for casual greetings or non-research queries
    casual_greetings = {"hi", "hello", "hey", "greetings", "namaste", "good morning", "good evening", "how are you"}
    
    # If the exact match is a greeting, or if it's very short and contains a greeting
    if raw_query in casual_greetings or (len(raw_query.split()) < 3 and any(greet in raw_query for greet in casual_greetings)):
        logger.info("Casual greeting detected. Routing to plain_chat.")
        return "plain"
