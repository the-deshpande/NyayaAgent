...
class ChatMemoryStore:
    ...
    def get_recent_sessions(self, limit: int = 6) -> list[dict]:
        """Fetch the most recent sessions and their first user message to use as a title."""
        with contextlib.closing(self._connect()) as c:
            rows = c.execute(
                "SELECT session_id, updated_at FROM sessions ORDER BY updated_at DESC LIMIT ?", (limit,)
            ).fetchall()
            ...
            for r in rows:
                ...
                if msg_row and msg_row["content"]:
                    ...
                    # Remove the persona injection prefix if present
                    import re
                    content = re.sub(r'^\[User Persona: .*?\]\n', '', content).strip()
                    words = content.split()
                    title = " ".join(words[:5])
                    if len(words) > 5:
                        title += "..."
    ...
    def prune_sessions(self, limit: int = 6):
        """Keep only the most recent `limit` sessions, deleting older ones."""
        with contextlib.closing(self._connect()) as c:
            # Find the recent sessions to keep
            rows = c.execute(
            ...
            c.execute(f"DELETE FROM messages WHERE session_id NOT IN ({placeholders})", keep_ids)
            c.execute(f"DELETE FROM sessions WHERE session_id NOT IN ({placeholders})", keep_ids)
            c.commit()
